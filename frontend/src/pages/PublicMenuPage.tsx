...
function PublicMenuContent({
  ...
  const handleAddProduct = (product: PublicMenu['categorias'][number]['productos'][number]) => {
    if (!product.available || product.out_of_operational_stock || !menu.accepts_orders) return;
  ...
  const featuredProducts = categories.flatMap((category) => category.productos.filter((product) => product.has_promotion && product.promotion_featured));
    ...
    <main className="min-h-screen bg-slate-100 pb-28">
      <section className={`relative overflow-hidden px-4 pb-7 pt-5 ${headerText}`} style={{ backgroundColor: primaryColor }}>
        ...
        <div className="relative mx-auto max-w-3xl">
          ...
          <div className="mt-5 rounded-2xl bg-white/90 p-4 text-sm text-slate-800">
            <p className="font-semibold">{menu.opening_status_message}</p>
            <p className="mt-1">Metodos: {menu.delivery_enabled ? 'Envio' : ''}{menu.delivery_enabled && menu.pickup_enabled ? ' · ' : ''}{menu.pickup_enabled ? 'Retiro' : ''}</p>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-3xl space-y-6 px-4 pt-6">
        {featuredProducts.length > 0 ? (
          <section className="space-y-3">
            <h2 className="text-xl font-semibold text-slate-900">Ofertas</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              {featuredProducts.slice(0, 4).map((product) => (
                <a key={product.id} href={`#product-${product.id}`} className="rounded-3xl border border-emerald-200 bg-emerald-50 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">{product.promotion_name || 'Promocion'}</p>
                  <p className="mt-2 font-semibold text-slate-900">{product.name}</p>
                  <p className="mt-1 text-sm text-slate-600">{formatPrice(product.final_price)}</p>
                </a>
              ))}
            </div>
          </section>
          ...
          <section key={category.id} className="space-y-3">
            ...
            <div className="space-y-3">
              {category.productos.map((product) => (
                <article id={`product-${product.id}`} key={product.id} className={`scroll-mt-4 flex gap-3 rounded-3xl bg-white p-3 shadow-sm transition duration-300 ${addedProductId === product.id ? 'scale-[1.01] ring-2 ring-emerald-300' : ''} ${!product.available ? 'opacity-70' : ''}`}>
                  ...
                  <div className="min-w-0 flex-1 py-1">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="shrink-0 text-right">
                        {product.has_promotion ? <p className="text-xs text-slate-400 line-through">{formatPrice(product.original_price)}</p> : null}
                        <p className="text-sm font-semibold" style={{ color: primaryColor }}>{formatPrice(product.final_price)}</p>
                      </div>
                    </div>
                    {product.has_promotion ? <span className="mt-2 inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">{product.discount_percentage ? `${Number(product.discount_percentage).toFixed(0)}% OFF` : product.promotion_name}</span> : null}
                    {product.description ? <p className="mt-2 line-clamp-2 text-sm leading-5 text-slate-500">{product.description}</p> : null}
                    <div className="mt-3 flex justify-end">
                      {product.available && !product.out_of_operational_stock ? (
                        <button type="button" disabled={!menu.accepts_orders} onClick={() => handleAddProduct(product)} className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-3 py-2 text-xs font-semibold text-white transition hover:bg-slate-800 disabled:opacity-50">
                          <Plus size={15} />
                          Agregar
                        </button>
                      ) : (
                        <span className="rounded-2xl bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-500">{product.out_of_operational_stock ? 'Sin stock' : 'Agotado'}</span>
                      )}
                    </div>
                  </div>
                </article>
      ...
      <CartDrawer open={cartOpen} businessId={menu.id} businessPhone={menu.phone} acceptsOrders={menu.accepts_orders} minimumOrder={Number(menu.minimum_order)} deliveryFee={Number(menu.delivery_fee)} freeDeliveryFrom={menu.free_delivery_from ? Number(menu.free_delivery_from) : null} deliveryEnabled={menu.delivery_enabled} pickupEnabled={menu.pickup_enabled} requirePhone={menu.require_customer_phone} requireAddress={menu.require_delivery_address} estimatedDeliveryMinutes={menu.estimated_delivery_minutes} estimatedPickupMinutes={menu.estimated_pickup_minutes} statusMessage={menu.opening_status_message} formatPrice={formatNumberPrice} onClose={() => setCartOpen(false)} />
    </main>
  );
}
